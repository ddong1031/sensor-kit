...
public class SensoroCameraConnection {
    ...
    private BluetoothGattCallback bluetoothGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            Log.e("SensoroCameraConnection", "newState::" + newState);
            if (newState == BluetoothProfile.STATE_CONNECTED) {//连接成功
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    Log.e("SensoroCameraConnection", "连接成功");
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {//发现服务
                ...
                if (bluetoothLEHelper4.checkGattServices(gattServiceList, BluetoothLEHelper4.GattInfo
                    ...
                    bluetoothLEHelper4.listenDescriptor(BluetoothLEHelper4.GattInfo.SENSORO_CAMERA_READ_CHAR_UUID);
                } else {
        ...
        public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
            ...
            Log.d("SensoroCameraConnection", "----uuid--->"+descriptor.getUuid());
        ...
        private void parseCharacteristicRead(BluetoothGattCharacteristic characteristic, int status) {
            if (characteristic.getUuid().equals(BluetoothLEHelper4.GattInfo.SENSORO_CAMERA_READ_CHAR_UUID)) {
                if (status == BluetoothGatt.GATT_SUCCESS) {
    ...
    private void parseChangedData(BluetoothGattCharacteristic characteristic) throws InvalidProtocolBufferException {
        ...
        if (uuid.equals(BluetoothLEHelper4.GattInfo.SENSORO_CAMERA_READ_CHAR_UUID)) { //出现先change后read情况
            if (byteBuffer == null) {
    ...
    private void parseStationData(byte[] data) throws Exception {
        ...
        SensoroCamera sensoroCamera = (SensoroCamera) bleDevice;
    ...
    private void writePassword(String newPassword, SensoroWriteCallback writeCallback) {
        ...
        int resultCode = bluetoothLEHelper4.setPassword(newPassword, BluetoothLEHelper4.GattInfo
                .SENSORO_CAMERA_WRITE_CHAR_UUID);
        if (resultCode != ResultCode.SUCCESS) {
            writeCallback.onWriteFailure(resultCode, CmdType.CMD_NULL);
        }
    }
